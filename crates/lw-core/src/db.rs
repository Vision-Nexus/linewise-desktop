...
impl Database {
    ...
    pub fn update_upload_session_uri(
        &self,
        id: &str,
        session_uri: &str,
    ) -> Result<(), DbError> {
        let conn = self.conn.lock().expect("db mutex poisoned");
        conn.execute(
            "UPDATE upload_queue SET gcs_session_uri = ?1, updated_at = datetime('now') WHERE id = ?2",
            rusqlite::params![session_uri, id],
        )?;
        Ok(())
    }
    ...
    pub fn get_pending_uploads(&self) -> Result<Vec<UploadTask>, DbError> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, local_path, filename, size, mime_type, tenant_id, project_id,
                    document_id, gcs_session_uri, bytes_uploaded, state, error_message,
